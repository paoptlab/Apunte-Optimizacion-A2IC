#!/usr/bin/env python3
"""Regenera los datos de la presentación desde el notebook solución.

El script ejecuta únicamente las celdas numéricas necesarias (omite instalación,
Colab y gráficos Plotly), exporta JSON/JavaScript y, opcionalmente, recompila el
PDF. De este modo el notebook sigue siendo la única fuente de verdad.
"""
...
DEFAULT_NOTEBOOK = ROOT.parent / "RP-Modelos-Avanzados" / "Modelos-avanzados-solucion.ipynb"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Actualiza la presentación con resultados recalculados del notebook."
    )
    parser.add_argument(
        "--notebook",
        type=Path,
        default=DEFAULT_NOTEBOOK,
        help="Notebook solución que actúa como fuente (por defecto: %(default)s).",
    )
    parser.add_argument(
        "--destino",
        type=Path,
        default=DEFAULT_DECK,
        help="Carpeta de la presentación (por defecto: %(default)s).",
    )
    parser.add_argument(
        "--licencia",
        type=Path,
    ...
    parser.add_argument(
        "--pdf",
        action="store_true",
        help="Además recompila Actividad-1-A2IC.pdf.",
    )
    parser.add_argument(
        "--salida-pdf",
        type=Path,
        default=DEFAULT_PDF,
        help="Destino del PDF al usar --pdf (por defecto: %(default)s).",
    )
    parser.add_argument(
        "--sin-standalone",
        action="store_true",
        help="No regenera standalone.html.",
    )
...
def export_payload(payload: dict[str, Any], destination: Path) -> None:
    ...
    atomic_write(destination / "datos-presentacion.json", pretty)
    atomic_write(destination / "nb-data.js", f"window.NB = {compact};\n")
    print(f"Datos JSON: {destination / 'datos-presentacion.json'}")
...
def main() -> int:
    ...
    export_payload(payload, destination)

    if not args.sin_standalone:
        run_helper(ROOT / "generar_standalone.py", "--carpeta", str(destination))
    if args.pdf:
        run_helper(
            ...
            str(destination),
            "--salida",
            str(args.salida_pdf.expanduser().resolve()),
        )
